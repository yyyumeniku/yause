...
public class GuiIngameMenuVoxelBox extends GuiIngameMenu {
    ...
    public void initGui() {
        ...
        try {
            ...
            if (VoxelMenuConfig.showPlaytime && com.thevoxelbox.yause.VoxelMenu.ftbUtilitiesInstalled) {
                ...
                this.basePlayTicks = v == null ? 0L : v.longValue();
            } else {
                this.basePlayTicks = 0L;
            }
        } catch (Throwable ignored) {
            this.basePlayTicks = 0;
        }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        String buttonText = button.displayString;
            ...
            // Open Statistics directly; leave button highlight behavior unchanged so the menu looks normal.
        ...
        try {
            ...
            Object val = ftbuReadStatMethod.invoke(stats, StatList.PLAY_ONE_MINUTE);
                if (val == null) {
                    ...
                    ftbuPlaytimeMissingLogged = true;
                    com.thevoxelbox.yause.VoxelMenu.LOGGER.info("FTBU: playtime stat read returned null (stats object: {}). Attempting vanilla stat fallback.", stats == null ? "null" : stats.getClass().getName());
                    try {
                        if (this.mc != null && this.mc.player != null) {
                            int vanillaTicks = this.mc.player.getStatFileWriter().readStat(StatList.PLAY_ONE_MINUTE);
                            com.thevoxelbox.yause.VoxelMenu.LOGGER.debug("FTBU fallback (null stat): using vanilla playtime stat value: {}", vanillaTicks);
                            return (long) vanillaTicks;
                        }
                    } catch (Throwable ignore) { }
                    return null;
                }
            ...
            try {
                if (this.mc != null && this.mc.player != null) {
                    int vanillaTicks = this.mc.player.getStatFileWriter().readStat(StatList.PLAY_ONE_MINUTE);
                    com.thevoxelbox.yause.VoxelMenu.LOGGER.debug("FTBU fallback: using vanilla playtime stat value: {}", vanillaTicks);
                    return (long) vanillaTicks;
                }
            } catch (Throwable ignore) {
                // Nothing else to do — gracefully return null if vanilla also fails
            }
            return null;
        }
    }
